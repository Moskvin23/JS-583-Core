...
try {
  ...
  console.log("Sorry, but you entered not numbers, but letters(((");
}
...
function checkAge() {
  ...
  if (!ageUser) {
    throw new Error(
      "Sorry User, but you didn`t enter anything , please enter your age",
      " "
    );
  } else if (isNaN(ageUser)) {
    throw new Error(
      "Sorry User, but this isn`t a number, please enter your age",
      " "
    );
  } else if (ageUser <= 14) {
...
function showUser(id) {
  if (id < 0) {
    throw new Error(
      "Sorry User,but you must enter a number for your ID : " + id
    );
  }
  return { id: id };
}
